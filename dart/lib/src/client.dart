...
class ArticleideasValidationException implements Exception {
  final List<String> errors;

  ArticleideasValidationException(this.errors);

  @override
  String toString() => 'ArticleideasValidationException: ${errors.join("; ")}';
}
...
/// Article Ideas Generator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/articleideas?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [topic] (required) - The topic for which you want to generate article ideas (up to 50 characters)
class ArticleideasClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'topic': ValidationRule(type: 'string', required: true),
  };

  ArticleideasClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/articleideas',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [ArticleideasValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw ArticleideasValidationException(errors);
    }
  }

  /// Execute a request to the Article Ideas Generator API.
  ///
  /// Parameters are validated before sending the request.
  Future<ArticleideasResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw ArticleideasException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return ArticleideasResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw ArticleideasException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw ArticleideasException('Resource not found');
      } else {
        throw ArticleideasException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is ArticleideasException) rethrow;
      throw ArticleideasException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Article Ideas Generator API client.
class ArticleideasException implements Exception {
  final String message;

  ArticleideasException(this.message);

  @override
  String toString() => 'ArticleideasException: $message';
}
